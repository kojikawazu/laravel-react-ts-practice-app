...
const DropdownLink = ({ className = '', children, ...props }: InertiaLinkProps) => {
    return (
        <Link
            {...props}
            className={
                'block w-full px-4 py-2 text-start text-sm leading-5 text-amber-700 hover:bg-amber-100 focus:outline-none focus:bg-amber-100 transition duration-150 ease-in-out ' +
                className
            }
        >
            {children}
